...
const usersCond = users.reduce((acc, item) => {
  const balance = parseFloat(item.balance.replace(/[^\d.-]/g, ''));
  if (balance > 2000) acc.push({ name: item.name, phone: item.phone });
  return acc;
}, []);
console.log(usersCond);

//Знайти суму всіх балансів користувачів
const sum = users.reduce(
  (acc, item) => acc + parseFloat(item.balance.replace(/[^\d.-]/g, '')),
  0
);
console.log(sum);
